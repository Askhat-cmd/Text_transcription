...
def _build_schema_validation(smoke: dict[str, Any]) -> dict[str, Any]:
    checks = smoke.get("api_checks") if isinstance(smoke.get("api_checks"), dict) else {}
    endpoint_contracts: dict[str, Any] = {}
    for endpoint in ("/api/status", "/api/registry", "/api/dashboard", "/api/dashboard/"):
        ...
        endpoint_contracts[endpoint] = {
            ...
            "body_is_non_empty": bool(body) if isinstance(body, (dict, list, str)) else body is not None,
        }

    schema_validation_passed = bool(smoke.get("admin_consistency_passed")) and str(smoke.get("admin_runtime_status")) == "passed"
    return {
        "schema_version": "admin_schema_validation_hf2_v1",
        "source_prd": "PRD-046.0.7.2-HF2",
        "generated_at": _utc_now_iso(),
        "admin_runtime_status": str(smoke.get("admin_runtime_status") or ""),
        "admin_consistency_passed": bool(smoke.get("admin_consistency_passed")),
        "schema_validation_passed": schema_validation_passed,
        "focus_source_id": smoke.get("focus_source_id"),
        "focus_blocks_count": smoke.get("focus_blocks_count"),
        "dashboard_blocks_count": smoke.get("dashboard_blocks_count"),
        "chroma_count": smoke.get("chroma_count"),
...
def _build_payload_shape_sanitized(smoke: dict[str, Any]) -> dict[str, Any]:
    checks = smoke.get("api_checks") if isinstance(smoke.get("api_checks"), dict) else {}
    ...
    return {
        "schema_version": "admin_payload_shape_sanitized_hf2_v1",
        "source_prd": "PRD-046.0.7.2-HF2",
        "generated_at": _utc_now_iso(),
...
def _implementation_report(snapshot: dict[str, Any], smoke: dict[str, Any], schema_validation: dict[str, Any], no_mutation: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- server_started_by_hf1: `{smoke.get('server_started_by_hf1')}`",
        ...
        f"- schema_validation_passed: `{schema_validation.get('schema_validation_passed')}`",
        "",
        "## Data expectations",
    ...
    return render_markdown_report("PRD-046.0.7.2-HF2 IMPLEMENTATION REPORT", lines)


def _admin_smoke_report(smoke: dict[str, Any], schema_validation: dict[str, Any]) -> str:
    lines = [
        "## Status",
        f"- admin_runtime_status: `{smoke.get('admin_runtime_status')}`",
        ...
        f"- schema_validation_passed: `{schema_validation.get('schema_validation_passed')}`",
        f"- server_launch_mode: `{smoke.get('server_launch_mode')}`",
        f"- detected_existing_server: `{smoke.get('detected_existing_server')}`",
        "",
        "## Endpoints",
        f"- /api/status: `{_endpoint_status(smoke, '/api/status')}`",
        f"- /api/registry: `{_endpoint_status(smoke, '/api/registry')}`",
    ...
    return render_markdown_report("PRD-046.0.7.2-HF2 ADMIN LIVE SMOKE REPORT", lines)


def _next_prd_report(snapshot: dict[str, Any], smoke: dict[str, Any]) -> str:
    final_status = str(snapshot.get("final_status") or "failed")
    if final_status == "passed":
        next_prd = "PRD-046.1 - Diagnostic Center v1 Readiness / Architecture PRD"
    elif final_status == "done_with_admin_schema_blocker":
        next_prd = "PRD-046.0.7.2-HF3 - Admin Schema/API Contract Fix v1"
    elif final_status == "done_with_admin_launch_blocker":
        next_prd = "PRD-046.0.7.2-HF3 - Admin Backend Launch/Env Fix v1"
    else:
        next_prd = "PRD-046.0.7.2-HF3 - Admin API Runtime Availability Fix v1"
    lines = [
        "## Recommendation",
        f"- next_prd: `{next_prd}`",
        f"- final_status: `{final_status}`",
    ...
    return render_markdown_report("PRD-046.0.7.2-HF2 NEXT PRD RECOMMENDATION", lines)


def _extract_chroma_count_from_payload(payload: Any) -> int | None:
    if not isinstance(payload, dict):
        return None
...
def _load_local_chroma_count_proof(expected_count: int) -> dict[str, Any]:
    candidates = [
    ...
    for path in candidates:
        ...
        try:
            ...
            continue
        count = _extract_chroma_count_from_payload(payload)
        if count is None:
            continue
        if count == expected_count:
            return {"available": True, "count": count, "path": str(path.relative_to(REPO_ROOT))}
        return {"available": True, "count": count, "path": str(path.relative_to(REPO_ROOT))}
    return {"available": False, "count": None, "path": None}
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload_shape = _build_payload_shape_sanitized(smoke)

    write_json(out_dir / "admin_live_smoke_hf2.json", smoke)
    write_json(out_dir / "admin_schema_validation_hf2.json", schema_validation)
    write_json(out_dir / "admin_payload_shape_sanitized.json", payload_shape)
    ...
    snapshot["schema_version"] = "post_apply_quality_gate_hf2_v1"
    write_json(out_dir / "post_apply_quality_gate_hf2.json", snapshot)

    write_text(
        reports_dir / "PRD-046.0.7.2-HF2_IMPLEMENTATION_REPORT.md",
        _implementation_report(snapshot, smoke, schema_validation, no_mutation),
    )
    write_text(
        reports_dir / "PRD-046.0.7.2-HF2_ADMIN_LIVE_SMOKE_REPORT.md",
        _admin_smoke_report(smoke, schema_validation),
    )
    write_text(
        reports_dir / "PRD-046.0.7.2-HF2_NEXT_PRD_RECOMMENDATION.md",
        _next_prd_report(snapshot, smoke),
    )

    return {
        "status": str(snapshot.get("final_status") or "failed"),
        "admin_live_smoke_hf2": smoke,
        "admin_schema_validation_hf2": schema_validation,
        "post_apply_quality_gate_hf2": snapshot,
        "no_mutation_proof": no_mutation,
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="PRD-046.0.7.2-HF2 admin live gate closure runner.")
    parser.add_argument("--source-prd", default="PRD-046.0.7.2-HF2")
    ...
    parser.add_argument("--out-dir", default="TO_DO_LIST/logs/PRD-046.0.7.2-HF2")
