...
PRD_ID = "PRD-047.42-APPLY-7"
OUT_DIR = REPO_ROOT / "TO_DO_LIST" / "logs" / PRD_ID
NORMALIZED_TIMESTAMP = "normalized_for_prd_047_42_apply_7"
PROTECTED_FILES = [
...
def build_no_mutation_proof() -> str:
    ...
    lines = [
        ...
        "",
        f"- PRD: `{PRD_ID}`",
        "- Scope rule: only `_call_llm` slice 1 moved; previously accepted helper/mixin/admin/contract files stay unchanged.",
...
def build_next_recommendation() -> str:
    return "\n".join(
        [
            ...
            "  - the next candidate should continue with the smallest remaining pre-provider helper edge that still avoids provider dispatch and response parsing;",
