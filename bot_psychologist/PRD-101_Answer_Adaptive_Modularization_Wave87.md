# PRD-101: `answer_adaptive.py` Modularization (Wave 87)

## Context
Fast-path stage still consumed several runtime-internal dependencies from `answer_adaptive.py` (debug bootstrap, fast-path mode directive, phase8 suffix builder, and mode-directive wiring).

## Scope (Wave 87)
Further narrow `_run_fast_path_stage(...)` by localizing these internals inside `runtime_misc_helpers.py`.

## Objectives
1. Shrink fast-path argument contract and facade imports.
2. Preserve fast-path behavior and trace payloads.
3. Validate with expanded targeted and full suites.

## Technical Design
Update `bot_agent/adaptive_runtime/runtime_misc_helpers.py`:
- remove `_run_fast_path_stage(...)` params:
  - `detect_fast_path_reason_fn`
  - `apply_fast_path_debug_bootstrap_fn`
  - `build_fast_path_mode_directive_fn`
  - `build_mode_directive_fn`
  - `build_phase8_context_suffix_fn`
- localize imports and direct calls:
  - from `routing_stage_helpers`: `_apply_fast_path_debug_bootstrap`, `_build_fast_path_mode_directive`, `_build_phase8_context_suffix`
  - from `state_helpers`: `_detect_fast_path_reason`
  - from `decision`: `build_mode_directive`

Update `bot_agent/answer_adaptive.py`:
- remove obsolete fast-path call args listed above.
- remove now-unused facade imports for these helpers.

## Test Plan
Expanded targeted:
- `tests/regression/test_no_level_based_prompting.py`
- `tests/test_retrieval_pipeline_simplified.py`
- `tests/integration/test_degraded_mode_without_retrieval.py`
- `tests/e2e/test_degraded_retrieval_case.py`
- `tests/integration/test_sparse_output_triggers_regeneration_hint.py`
- `tests/integration/test_runtime_validation_receives_query_v1031.py`
- `tests/integration/test_routing_recalibration_for_exploratory_queries.py`
- `tests/integration/test_adaptive_stream_contract.py`
- `tests/test_sse_payload.py`
- `tests/unit/test_curious_not_auto_informational.py`
- `tests/unit/test_user_level_adapter_removed.py`
- `tests/unit/test_sd_legacy_final_cleanup_prompt_context.py`
- `tests/unit/test_query_is_passed_to_output_validation_policy_v1031.py`
- `tests/integration/test_generation_validation_separation.py`

Full:
- `pytest -q tests --maxfail=1`
