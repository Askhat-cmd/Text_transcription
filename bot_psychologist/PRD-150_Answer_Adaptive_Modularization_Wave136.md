...
## Context
`response_utils.py` still contained large full/fast success-stage orchestration blocks, making the module harder to navigate and slower for future cleanup waves.
...
- Move success-stage helper functions from `response_utils.py` into the new module:
...
- Keep `response_utils.py` as compatibility surface by importing moved helpers.
...
2. `response_utils.py` no longer contains moved function bodies and re-exports them via imports.
