"""Contracts for PRD-046.1.35-HF1 creator live evidence + RAG-to-writer repair."""
...
@dataclass
class HF1Scorecard:
    schema_version: str = "creator_live_evidence_rag_repair_scorecard_v1"
    prd_id: str = PRD_ID
    final_status: str = "blocked"
    decision: str = "hf1_evidence_incomplete_fix_required"
    source_gate: str = "blocked"
    service_readiness_gate: str = "blocked"
    creator_identity_gate: str = "blocked"
    creator_live_turn_gate: str = "blocked"
    botdb_query_gate: str = "blocked"
    rag_delivery_gate: str = "blocked"
    context_writer_delivery_gate: str = "blocked"
    ui_trace_alignment_gate: str = "blocked"
    ...
    def __post_init__(self) -> None:
        self.schema_version = _as_str(self.schema_version, "creator_live_evidence_rag_repair_scorecard_v1")
        self.prd_id = _as_str(self.prd_id, PRD_ID)
        self.final_status = _as_str(self.final_status, "blocked")
        self.decision = _as_str(self.decision, "hf1_evidence_incomplete_fix_required")
        self.source_gate = _as_str(self.source_gate, "blocked")
        self.service_readiness_gate = _as_str(self.service_readiness_gate, "blocked")
        self.creator_identity_gate = _as_str(self.creator_identity_gate, "blocked")
        ...
        self.rag_delivery_gate = _as_str(self.rag_delivery_gate, "blocked")
        self.context_writer_delivery_gate = _as_str(self.context_writer_delivery_gate, "blocked")
        self.ui_trace_alignment_gate = _as_str(self.ui_trace_alignment_gate, "blocked")
...
@dataclass
class HF1Decision:
    schema_version: str = "creator_live_evidence_rag_repair_decision_v1"
    prd_id: str = PRD_ID
    final_status: str = "blocked"
    decision: str = "hf1_evidence_incomplete_fix_required"
    blockers: list[str] = field(default_factory=list)
    ...
    def __post_init__(self) -> None:
        self.schema_version = _as_str(self.schema_version, "creator_live_evidence_rag_repair_decision_v1")
        self.prd_id = _as_str(self.prd_id, PRD_ID)
        self.final_status = _as_str(self.final_status, "blocked")
        self.decision = _as_str(self.decision, "hf1_evidence_incomplete_fix_required")
        self.blockers = [str(item) for item in _safe_list(self.blockers)]
