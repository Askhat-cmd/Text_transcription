...
_IMPERFECT_SELF_RE = re.compile(
    r"(не\s+справлюсь|опоздал\w*|ничего\s+не\s+получит\w+|я\s+недостаточ\w*|со\s+мной\s+что-?то\s+не\s+так|все\s+упустил|уже\s+поздно)",
    re.IGNORECASE,
)
_DRIVER_RE = re.compile(
    r"(должен\w*|надо\s+быть\s+сильн\w+|надо\s+лучше|старать\w+|быстре\w+)",
    re.IGNORECASE,
)
_SHORT_SUPPORT_RE = re.compile(r"(без\s+анализ\w+|пару\s+спокойн\w+\s+слов|я\s+устал)", re.IGNORECASE)
...
def render_kernel_prompt_block(
    *,
    ...
    principle_lines = "\n".join(f"- {line}" for line in active.principles[:4])
    boundary_lines = "\n".join(f"- {line}" for line in active.boundaries[:5])
