# Multiagent Launch Checklist (Чеклист первого живого запуска мультиагентной системы)
...
- [ ] `pytest tests/multiagent/ -q` -> весь пакет зелёный.
...
- [ ] Результат первого запуска отражён в `CHANGELOG.md`.

## Rollback (Откат)

Если новый режим ведёт себя нестабильно:
