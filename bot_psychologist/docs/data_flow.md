...
**Что содержит**:
- сточник данных (voice_bot_pipeline)
...
4. **Кэширование**
   ...
   - ндексы для быстрого поиска по ID
...
2. сторическая фильтрация по `complexity_score`
3. В active NEO runtime данный контур не используется
...
**Процесс**:
1. звлечение концептов из запроса
...
**спользуемые поля**:
- Все поля Phase 1-3
- стория диалога из `.cache_bot_agent/conversations/`
...
[GraphClient] → Построение Knowledge Graph
    ↓
[Bot Agent] → спользование данных для ответов
    ↓
