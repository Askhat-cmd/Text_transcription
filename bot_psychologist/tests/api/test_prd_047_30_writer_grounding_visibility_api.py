...
def test_multiagent_trace_endpoint_includes_writer_grounding_visibility_v1() -> None:
    _reset_store()
    store = get_session_store()
    store.save_multiagent_debug(
        session_id="writer-grounding-session",
        turn_index=1,
        debug={
    ...
    with TestClient(app, base_url="http://localhost") as client:
        response = client.get(
            "/api/debug/session/writer-grounding-session/multiagent-trace",
            headers=DEV_HEADERS,
        )

    assert response.status_code == 200
    payload = response.json()
    assert payload["writer_grounding_visibility_v1"]["reason"] == "non_kb_emotional_support_turn"
    assert payload["runtime_trace_summary_v1"]["writer_grounding_visibility_v1"]["kb_visible_to_writer"] is False
