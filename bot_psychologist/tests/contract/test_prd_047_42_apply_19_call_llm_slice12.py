...
from TO_DO_LIST.tools import run_prd_047_42_apply_19_call_llm_slice12 as runner
...
def test_runner_normalizes_snapshot_timestamp() -> None:
    ...
    assert all(case["last_debug"]["duration_ms"] == 123 for case in payload["cases"])


def test_compare_snapshot_payloads_reports_user_prompt_identity() -> None:
    before_payload = {"cases": [{"case": "a", "last_debug": {"user_prompt": "same"}}]}
    after_payload = {"cases": [{"case": "a", "last_debug": {"user_prompt": "same"}}]}
    ...
    assert comparison["all_user_prompts_match"] is True
    assert comparison["cases"][0]["before_user_prompt_sha1"] == comparison["cases"][0]["after_user_prompt_sha1"]
...
def test_runner_writes_expected_reports(tmp_path: Path) -> None:
    ...
    assert all(case["last_debug"]["duration_ms"] == 123 for case in snapshot["cases"])
