...
def test_literal_markdown_echo_case_exercises_the_echo_mismatch_outcome() -> None:
    after_payload = runner.build_after_snapshot()
    cases_by_name = {case["case"]: case for case in after_payload["cases"]}

    case = cases_by_name["literal_markdown_echo"]
    assert case["last_debug"]["final_answer_shape"] == "literal_markdown_echo"
    assert case["last_debug"]["format_request_repair_applied"] is True
    keys = list(case["last_debug"].keys())
    assert keys.index("format_request_repair_applied") < keys.index("final_answer_shape")
...
def test_runner_writes_expected_reports_and_grep_proof(tmp_path: Path) -> None:
    ...
    expected_names = {
        "enforce_slice4_snapshot_before.json",
        "enforce_slice4_snapshot_after.json",
    ...
    before_snapshot = json.loads((tmp_path / "enforce_slice4_snapshot_before.json").read_text(encoding="utf-8"))
    after_snapshot = json.loads((tmp_path / "enforce_slice4_snapshot_after.json").read_text(encoding="utf-8"))
    ...
    assert "`self.last_debug` writes found in window: `2`" in grep_proof
    ...
    assert "APPLY-20/21/22/23 logs diff result: `0 changed paths`" in no_mutation
