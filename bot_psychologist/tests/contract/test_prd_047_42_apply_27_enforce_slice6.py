...
def test_template_leakage_case_exercises_the_practice_forbidden_outcome_with_correct_key_order() -> None:
    after_payload = runner.build_after_snapshot()
    cases_by_name = {case["case"]: case for case in after_payload["cases"]}

    case = cases_by_name["template_leakage_practice_forbidden"]
    assert case["last_debug"]["final_answer_shape"] == "template_repair_deferred_to_gate"
    assert case["last_debug"]["template_leakage_repair_deferred_to_gate"] is True
    keys = list(case["last_debug"].keys())
    assert keys.index("template_leakage_repair_deferred_to_gate") < keys.index("final_answer_shape")
...
def test_runner_writes_expected_reports_and_grep_proof(tmp_path: Path) -> None:
    ...
    expected_names = {
        "enforce_slice6_snapshot_before.json",
        "enforce_slice6_snapshot_after.json",
    ...
    before_snapshot = json.loads((tmp_path / "enforce_slice6_snapshot_before.json").read_text(encoding="utf-8"))
    after_snapshot = json.loads((tmp_path / "enforce_slice6_snapshot_after.json").read_text(encoding="utf-8"))
    ...
    assert "direct writes found in window: `1`" in grep_proof
    assert "Group 7" in grep_proof and "confirmed untouched" in grep_proof
    ...
    assert "APPLY-20..26 logs diff result: `0 changed paths`" in no_mutation
