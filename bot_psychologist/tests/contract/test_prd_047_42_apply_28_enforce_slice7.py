...
def test_runner_writes_expected_reports_and_grep_proof(tmp_path: Path) -> None:
    ...
    expected_names = {
        "enforce_slice7_snapshot_before.json",
        "enforce_slice7_snapshot_after.json",
    ...
    before_snapshot = json.loads((tmp_path / "enforce_slice7_snapshot_before.json").read_text(encoding="utf-8"))
    after_snapshot = json.loads((tmp_path / "enforce_slice7_snapshot_after.json").read_text(encoding="utf-8"))
    ...
    assert "direct writes found in window: `0`" in grep_proof
    assert "method end, untouched): `True`" in grep_proof
    ...
    assert "APPLY-20..27 logs diff result: `0 changed paths`" in no_mutation
