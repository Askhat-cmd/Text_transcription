...
def test_runner_writes_expected_reports_and_grep_proof(tmp_path: Path) -> None:
    ...
    expected_names = {
        "mvp_slice1_snapshot_before.json",
        "mvp_slice1_snapshot_after.json",
    ...
    before_snapshot = json.loads((tmp_path / "mvp_slice1_snapshot_before.json").read_text(encoding="utf-8"))
    after_snapshot = json.loads((tmp_path / "mvp_slice1_snapshot_after.json").read_text(encoding="utf-8"))
    ...
    assert "direct writes found in window: `4`" in grep_proof
    assert "dead-code `offer_repair_context` assignment" in grep_proof
    assert "True" in grep_proof
    ...
    assert "APPLY-20..28 logs diff result: `0 changed paths`" in no_mutation
