...
def test_runner_writes_expected_reports_and_grep_proof(tmp_path: Path) -> None:
    ...
    expected_names = {
        "mvp_slice2_snapshot_before.json",
        "mvp_slice2_snapshot_after.json",
    ...
    before_snapshot = json.loads((tmp_path / "mvp_slice2_snapshot_before.json").read_text(encoding="utf-8"))
    after_snapshot = json.loads((tmp_path / "mvp_slice2_snapshot_after.json").read_text(encoding="utf-8"))
    ...
    assert "direct writes found in window: `2`" in grep_proof
    assert "confirmed COMPUTED" in grep_proof and "`True`" in grep_proof
    assert "Method end" in grep_proof
    ...
    assert "APPLY-20..29 logs diff result: `0 changed paths`" in no_mutation
