...
def test_kb_boundary_calibration_is_strict(tmp_path: Path) -> None:
    ...
    kb = json.loads((out_dir / "kb_boundary_calibration_report.json").read_text(encoding="utf-8"))
    assert kb["kb_boundary_respect_hard_fail_detection_rate"] == 1.0
    assert kb["acceptable_candidate_raw_quote_hits"] == 0
    assert kb["acceptable_candidate_authority_citation_hits"] == 0
