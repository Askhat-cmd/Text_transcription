...
def test_limited_smoke_consolidation_normal_user_no_effect() -> None:
    ...
    payload = gate.build_normal_user_no_effect_cumulative(preflight["parsed"])
    assert payload["normal_user_controls_total"] >= 4
    assert payload["normal_user_apply_count_total"] == 0
    assert payload["normal_user_no_effect_cumulative_passed"] is True
