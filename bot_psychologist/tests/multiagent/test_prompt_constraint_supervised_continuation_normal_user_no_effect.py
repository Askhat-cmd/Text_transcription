...
def test_normal_user_no_effect_gate(tmp_path: Path) -> None:
    out_dir = tmp_path / "out"
    runner.run(argparse.Namespace(source_dir="TO_DO_LIST/logs/PRD-046.1.9", output_dir=str(out_dir), strict=True))
    payload = json.loads((out_dir / "supervised_continuation_normal_user_no_effect.json").read_text(encoding="utf-8"))
