...
def test_quality_comparison_is_clean(tmp_path: Path) -> None:
    out_dir = tmp_path / "out"
    runner.run(argparse.Namespace(source_dir="TO_DO_LIST/logs/PRD-046.1.9", output_dir=str(out_dir), strict=True))
    payload = json.loads((out_dir / "supervised_continuation_baseline_vs_test_apply.json").read_text(encoding="utf-8"))
    assert payload["cases_compared"] >= 6
    ...
    assert payload["provider_called_by_continuation_count"] == 0
