...
def _stub_adaptive_result(*_args, **_kwargs):
    return {
        ...
        "debug_trace": {
            ...
            "pipeline_stages": [{"name": "retrieval", "label": "Retrieval", "duration_ms": 3}],
            "config_snapshot": {
...
def test_trace_reflects_real_execution_only_with_sd_runtime_disabled(monkeypatch) -> None:
    ...
    trace = response.json().get("trace") or {}
    assert trace.get("recommended_mode") == "PRESENCE"
    assert trace.get("mode_reason") == "deterministic"
    ...
    assert "user_level" not in config_snapshot
    assert "sd_confidence_threshold" not in config_snapshot
