...
def client(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(config, "BOT_DB_PATH", tmp_path / "trace_planner.db", raising=False)
...
def test_api_trace_keeps_response_planner_payload(client: TestClient) -> None:
    session_id = "rp-trace-001"
    ...
    assert payload["trace"]["response_planner_version"] == "response_planner_v1"
    assert payload["trace"]["response_planner"]["next_move"] == "deepen_mechanism"
    assert payload["trace"].get("response_planner_error") is None
    ...
    assert last_trace["response_planner_version"] == "response_planner_v1"
    assert last_trace["response_planner"]["next_move"] == "deepen_mechanism"
