...
def _run(mode: str, log_dir: Path) -> tuple[int, dict]:
    script = Path(__file__).resolve().parents[1] / "scripts" / "run_prd_047_11_audit.py"
    proc = subprocess.run(
        [
            sys.executable,
            str(script),
...
def test_audit_runner_dry_mode_passes(tmp_path: Path) -> None:
    code, payload = _run("dry", tmp_path)
    assert code == 0
    assert payload.get("status") == "passed"


def test_audit_runner_direct_mode_passes(tmp_path: Path) -> None:
