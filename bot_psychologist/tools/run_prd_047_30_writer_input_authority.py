...
FIXTURE_PATH = REPO_ROOT / "TO_DO_LIST" / "fixtures" / "PRD-047.30" / "writer_input_authority_cases_ru.jsonl"
LOG_DIR_DEFAULT = REPO_ROOT / "TO_DO_LIST" / "logs" / "PRD-047.30"
API_KEY = "dev-key-001"
...
def _send_turn(base_url: str, *, session_id: str, query: str) -> tuple[str, dict[str, Any]]:
    status, response_payload = _http_json(
        "POST",
        f"{base_url}/api/v1/questions/adaptive",
        data=_ascii_body(query, session_id),
    )
    if status != 200:
        raise RuntimeError(f"adaptive_failed:{session_id}:{status}:{response_payload}")
    debug_payload = _wait_for_debug(base_url, session_id)
    return _answer_text(response_payload), debug_payload
...
def _evaluate_case(case: dict[str, Any], answer: str, debug_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    grounding = dict(debug_payload.get("writer_grounding_visibility_v1") or runtime_summary.get("writer_grounding_visibility_v1") or {})
    ...
    mismatches: list[str] = []
    if expected_constraints and active_constraints != expected_constraints:
        mismatches.append(f"latest_turn_constraints expected={expected_constraints} actual={active_constraints}")
    ...
    expected_reason = str(expected.get("visibility_reason", "") or "").strip()
    if expected_reason and str(grounding.get("reason", "") or "") != expected_reason:
        mismatches.append(f"visibility_reason expected={expected_reason} actual={grounding.get('reason')}")
...
def run_live_smoke(*, base_url: str, fixture_path: Path, log_dir: Path) -> dict[str, Any]:
    ...
    for case in cases:
        ...
        session_id = f"prd-047-30-{case_id.lower()}"
        for prelude_query in list(case.get("prelude", []) or []):
            _send_turn(base_url, session_id=session_id, query=str(prelude_query))
        answer, debug_payload = _send_turn(base_url, session_id=session_id, query=str(case.get("query", "")))
        row = _evaluate_case(case, answer, debug_payload)
        _write_json(exports_dir / f"{case_id}.json", row)
        results.append(row)

    report = {
        "schema_version": "prd_047_30_live_smoke_report_v1",
    ...
    for item in results:
        ...
        grounding = dict(item.get("writer_grounding_visibility_v1", {}) or {})
        lines.extend(
            [
                ...
                f"- visibility_reason: `{grounding.get('reason', '') or 'none'}`",
