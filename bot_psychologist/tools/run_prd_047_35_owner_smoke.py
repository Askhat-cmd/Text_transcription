...
OUT_DIR = REPO_ROOT / "TO_DO_LIST" / "logs" / "PRD-047.35"
...
SCENARIOS = [
    {
        "label": "A_owner_pilot_job_fear",
        "session_key": "owner-pilot-a",
        "turns": [
            "Привет. Что ты умеешь?",
            "Я боюсь, что если потеряю работу, то стану никому не нужен. Как распутать этот страх?",
            "Кажется, я боюсь даже начать, потому что если не получится — будет совсем больно.",
            "Спасибо, сейчас достаточно.",
        ],
    },
    {
        "label": "B_panic_support",
        "session_key": "owner-pilot-b",
        "turns": [
            "У моей жены иногда случаются панические атаки. Как мне ей помочь?",
            "Она говорит, что больше всего боится потерять контроль.",
            "Я понял. Не надо глубже, скажи коротко, что мне делать рядом с ней в моменте.",
        ],
    },
    {
        "label": "C_owner_diagnostic_db_question",
        "session_key": "owner-pilot-c",
        "turns": [
            "А что у тебя в базе говорится про панические атаки?",
            "Теперь ответь как обычный собеседник, без разговоров о базе.",
        ],
    },
    {
        "label": "D_creative_latest_turn",
        "session_key": "owner-pilot-d",
        "turns": [
            "Расскажи подробнее про этот механизм.",
            "Придумай короткую метафору на эту тему.",
...
def _post_turn(session: str, query: str) -> dict[str, Any]:
    return _request_json(
        ...
        headers={
            ...
            "X-Device-Fingerprint": "prd-047-35-live-fp",
        },
        timeout=220,
    )
...
def _turn_summary(
    *,
    ...
    hidden = _safe_dict(
        runtime_summary.get("hidden_knowledge_competence_v1")
        or truth.get("hidden_knowledge_competence_v1")
        or trace.get("hidden_knowledge_competence_v1")
    )
    return {
        ...
        "hidden_knowledge_competence_v1": hidden,
...
def main() -> int:
    ...
    run_prefix = f"prd-047-35-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    ...
    for scenario in SCENARIOS:
        ...
        for index, query in enumerate(scenario["turns"], start=1):
            ...
            print(
                ...
                f"profile={summary['answer_shape_profile']} "
                f"payload={summary['writer_payload_count']} "
                f"internal_language={summary['contains_internal_language']}"
            )
            sys.stdout.flush()
