...
def _build_thread(payload: dict[str, Any], *, case_id: str) -> ThreadState:
    return ThreadState(
        ...
        user_id=str(payload.get("user_id", "writer_prompt_replay_eval_user")),
        core_direction=str(payload.get("core_direction", "writer_prompt_replay_eval")),
...
def _build_diagnostic_card(payload: dict[str, Any]) -> DiagnosticCard:
    trace = DiagnosticCardTrace(
        ...
        builder="writer_prompt_replay_fixture",
...
def _run_runtime_smoke() -> list[dict[str, Any]]:
    ...
    smoke_cases = [
        ...
        },
        {
            ...
            "query": "Я снова чувствую недостаточность.",
    ...
    try:
        ...
        orch_module.writer_agent.write = AsyncMock(return_value="writer replay smoke answer")
        ...
        for case in smoke_cases:
            ...
            run_result = MultiAgentOrchestrator().run_sync(query=str(case["query"]), user_id="wpr_smoke")
            debug = _safe_dict(run_result.get("debug"))
            writer_contract = WriterContract(
                user_message=str(case["query"]),
                thread_state=case["thread"],
                memory_bundle=case["bundle"],
                diagnostic_card=DiagnosticCard.from_dict(_safe_dict(debug.get("diagnostic_card"))),
            )
            pilot_payload = _safe_dict(debug.get("planner_bridge_writer_contract_pilot"))
            if not pilot_payload:
                pilot_payload = {
                    "overlay": {
                        "candidate_constraints": {
                            "response_goal": case["thread"].response_goal,
                            "response_mode": case["thread"].response_mode,
            ...
            replay = build_writer_prompt_replay_runtime_shadow_v1(
                writer_contract=writer_contract,
                writer_contract_pilot=pilot_payload,
                diagnostic_card=writer_contract.diagnostic_card or DiagnosticCard.from_dict({}),
                thread_state=case["thread"],
                state_snapshot=case["state"],
            )
            results.append(
                {
                    ...
                    "replay_exists": bool(replay),
                    "activation_mode": replay.get("activation_mode"),
                    "apply_to_writer_contract": replay.get("apply_to_writer_contract"),
                    "apply_to_writer_prompt": replay.get("apply_to_writer_prompt"),
                    "apply_to_final_answer": replay.get("apply_to_final_answer"),
                    "provider_called": replay.get("provider_called"),
                }
            )
    finally:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for case in cases:
        ...
        bridge_payload = build_planner_bridge_compliance_runtime_shadow_v1(
            diagnostic_center_shadow=shadow,
            diagnostic_card=diagnostic_card,
            thread_state=thread,
            state_snapshot=state,
        )
        compliance_shadow = _safe_dict(bridge_payload.get("planner_bridge_compliance_shadow"))
        ...
        writer_contract_before = hashlib.sha256(_stable_json(writer_contract.to_dict()).encode("utf-8")).hexdigest()

        pilot_payload = build_planner_bridge_writer_contract_pilot_runtime_shadow_v1(
            writer_contract=writer_contract,
        ...
        pilot_override = _safe_dict(case.get("pilot_overlay_override"))
        if pilot_override:
            pilot_payload = _deep_merge(pilot_payload, pilot_override)
        ...
        writer_contract_after = hashlib.sha256(_stable_json(writer_contract.to_dict()).encode("utf-8")).hexdigest()

        expected = _safe_dict(case.get("expected_replay"))
        expected_ok, issues = _evaluate_expected(replay, expected)

        quality = _safe_dict(replay.get("quality"))
        comparison = _safe_dict(replay.get("comparison"))
        blocked_reasons = [str(item) for item in _safe_list(replay.get("blocked_reasons"))]

        pass_case = expected_ok and writer_contract_before == writer_contract_after
        if pass_case:
            cases_passed += 1

        quality_status = str(quality.get("quality_status", "blocked"))
        expected_quality_status = str(expected.get("quality.quality_status", ""))
        if quality_status == "blocked":
            hard_blocker_count += 1
        if quality_status == "needs_review" and expected_quality_status not in {"needs_review", ""}:
        ...
        if safety_case:
            ...
            if bool(quality.get("safety_ok", False)):
                safety_cases_passed += 1

        if not bool(quality.get("kb_boundary_ok", True)):
            kb_boundary_violation_count += 1
        if len(_safe_list(comparison.get("forbidden_field_hits"))) > 0:
            raw_kb_text_exposure_count += 1
        if not bool(quality.get("constraint_conflict_ok", True)):
            constraint_conflict_count += 1
        if "prompt_bloat_blocker" in blocked_reasons:
            prompt_bloat_blocker_count += 1

        if writer_contract_before != writer_contract_after:
            writer_contract_changed_by_replay_count += 1
        if bool(replay.get("apply_to_writer_prompt", False)):
            writer_prompt_changed_by_replay_count += 1
        if bool(replay.get("apply_to_final_answer", False)):
            final_answer_changed_by_replay_count += 1
        if bool(replay.get("provider_called", False)):
            ...
            trace_samples.append(
                {
                    ...
                    "warnings": _safe_list(replay.get("warnings")),
    ...
    runtime_smoke_ok = all(
        ...
        and item.get("replay_exists") is True
        and item.get("activation_mode") == "offline_replay_only"
        ...
        and item.get("provider_called") is False
        for item in runtime_smoke_cases
    )

    safety_replay_pass_rate = round(safety_cases_passed / safety_cases_total, 4) if safety_cases_total > 0 else 1.0
    ...
    no_mutation_proof = {
        ...
        "provider_called_by_writer_prompt_replay": provider_called_count > 0,
        "production_apply_performed": False,
        "writer_prompt_production_changed": False,
        "writer_contract_changed": writer_contract_changed_by_replay_count > 0,
        "final_answer_changed_by_replay": final_answer_changed_by_replay_count > 0,
        "diagnostic_center_active_in_writer": False,
        "planner_bridge_apply_to_writer": False,
        "pilot_apply_to_writer_contract": False,
        "replay_apply_to_writer_prompt": writer_prompt_changed_by_replay_count > 0,
        "replay_apply_to_final_answer": final_answer_changed_by_replay_count > 0,
    ...
    if cases_passed != len(cases):
        final_status = "done_with_replay_blocker"
    if (
        hard_blocker_count > 0
        or safety_replay_pass_rate < 1.0
        or kb_boundary_violation_count > 0
        or raw_kb_text_exposure_count > 0
        or constraint_conflict_count > 0
        or prompt_bloat_blocker_count > 0
        or writer_contract_changed_by_replay_count > 0
        or writer_prompt_changed_by_replay_count > 0
        or final_answer_changed_by_replay_count > 0
        or provider_called_count > 0
        or not runtime_smoke_ok
    ):
    ...
    scorecard = {
        ...
        "safety_replay_pass_rate": safety_replay_pass_rate,
        ...
        "writer_contract_changed_by_replay_count": writer_contract_changed_by_replay_count,
        "writer_prompt_changed_by_replay_count": writer_prompt_changed_by_replay_count,
        "final_answer_changed_by_replay_count": final_answer_changed_by_replay_count,
        "provider_called_count": provider_called_count,
        "runtime_smoke_ok": runtime_smoke_ok,
        "artifact_encoding_hygiene_passed": False,
        "writer_prompt_replay_ready": False,
        "final_status": final_status,
        "next_prd": "PRD-046.1.6",
    }

    _write_json(out_dir / "writer_prompt_replay_eval.json", {
        "schema_version": "writer_prompt_replay_eval_v1",
        "prd": PRD,
        "status": final_status,
        "generated_at": _utc_now(),
        "cases": case_results,
    })
    _write_json(out_dir / "writer_prompt_replay_scorecard.json", scorecard)
    _write_json(out_dir / "writer_prompt_replay_trace_samples.json", {
        "schema_version": "writer_prompt_replay_trace_samples_v1",
        "prd": PRD,
        "generated_at": _utc_now(),
        "samples": trace_samples,
    })
    _write_json(out_dir / "writer_prompt_replay_runtime_smoke.json", {
        "schema_version": "writer_prompt_replay_runtime_smoke_v1",
        "prd": PRD,
        "ok": runtime_smoke_ok,
        "generated_at": _utc_now(),
    ...
    scorecard["writer_prompt_replay_ready"] = (
        final_status == "passed" and scorecard["artifact_encoding_hygiene_passed"]
    )
    if final_status == "passed" and not scorecard["artifact_encoding_hygiene_passed"]:
        scorecard["final_status"] = "done_with_replay_blocker"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run writer prompt replay quality eval.")
    parser.add_argument(
        "--cases-file",
        default="bot_psychologist/tests/fixtures/writer_prompt_replay_cases.json",
    )
    parser.add_argument(
        "--out-dir",
        default="TO_DO_LIST/logs/PRD-046.1.5",
    )
    return parser
