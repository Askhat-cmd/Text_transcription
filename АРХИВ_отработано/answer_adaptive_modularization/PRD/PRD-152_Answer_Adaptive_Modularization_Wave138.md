...
## Context
After Wave 136/137, `response_utils.py (removed in Wave 142)` still mixed success helpers with failure/no-retrieval handling.
...
- Move from `response_utils.py (removed in Wave 142)`:
...
- Keep compatibility exports in `response_utils.py (removed in Wave 142)` via imports.

## Acceptance Criteria
1. Moved functions live in `response_failure_helpers.py`.
2. `response_utils.py (removed in Wave 142)` is reduced and still exposes previous callable names.
